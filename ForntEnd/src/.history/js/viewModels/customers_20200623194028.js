...
define([
  ...
  function CustomerViewModel() {
    ...
    $.getJSON("http://localhost:3000/api/countries").then((data) => {
      console.log(data);
      this.Country_Op = new ArrayDataProvider(data, {
        keyAttributes: "Country_ID",
      });
      /* self.Country_Op(
        new ListDataProviderView(Country, { dataMapping: dataMapCountry })
      );*/
    });
    ...
    this.valueChangeCountry = function (event) {
      console.log(JSON.stringify(this.val_Country));
      $.getJSON("http://localhost:3000/api/countries/" + 1 + "/cities").then(
        (data) => {
